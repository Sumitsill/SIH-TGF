...
const OTPVerification: React.FC<OTPVerificationProps> = ({
  ...
  //           </div>
  //           <div>
  //             <h1 className="text-3xl font-bold text-gray-900">HUNGERR</h1>
      ...
      <div className="max-w-md w-full">
        ...
        <div className="text-center mb-8">
          <div className="flex items-center justify-center space-x-3 mb-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-white">PRATIDWANDHI</h1>
              <p className="text-gray-300">Empowering Lives</p>
            </div>
          </div>
          <div className="bg-slate-800/60 border border-cyan-400/30 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4">
            <Shield className="h-8 w-8 text-cyan-400" />
          </div>
          <h2 className="text-2xl font-bold text-white mb-2">
            Verify Your Number
          </h2>
          <p className="text-gray-300">We've sent a 6-digit OTP to</p>
          <p className="font-semibold text-cyan-400">+91 {user?.phone}</p>
        </div>

        {/* OTP Verification Form */}
        <div className="bg-slate-800/40 backdrop-blur-sm border border-cyan-400/20 rounded-2xl shadow-xl p-8">
          <div className="mb-6">
            ...
            <div className="flex space-x-3 justify-center">
              {otp.map((digit, index) => (
                <input
                  ...
                  onKeyDown={(e) => handleKeyDown(e, index)}
                  onPaste={index === 0 ? handlePaste : undefined}
                  className={`w-12 h-14 text-center text-xl font-bold rounded-xl focus:ring-2 focus:ring-cyan-500 transition-all duration-200 
                  ${
                    error
                      ? "border-2 border-red-500 bg-red-900/30 text-red-300"
                      : "border border-gray-600 bg-slate-900/50 text-white"
                  } 
                  ${
                    digit ? "border-cyan-500 bg-cyan-900/30 text-cyan-300" : ""
                  }`}
                  maxLength={1}
                  disabled={isLoading}
                />
            ...
            {error && (
              <p className="mt-4 text-sm text-red-400 text-center flex items-center justify-center">
          ...
          <button
            ...
            ${
              isLoading || otp.some((digit) => digit === "")
                ...
                : "bg-gradient-to-r from-cyan-500 to-blue-500 text-white hover:from-cyan-600 hover:to-blue-600 transform hover:scale-105 shadow-lg hover:shadow-2xl"
            }`}
          >
            {isLoading ? (
              <>
          ...
          <div className="text-center">
            ...
            {canResend ? (
              <button
                onClick={handleResendOTP}
                disabled={isLoading}
                className="text-cyan-400 font-semibold hover:text-cyan-300 transition-colors duration-200 flex items-center justify-center space-x-1 mx-auto disabled:opacity-50"
              >
                <RefreshCw className="h-4 w-4" />
                <span>Resend OTP</span>
              </button>
          ...
          <button
            onClick={onBack}
            disabled={isLoading}
            className="w-full mt-4 py-3 border border-gray-600 text-gray-300 rounded-xl font-semibold hover:bg-slate-700/60 transition-colors duration-200 flex items-center justify-center space-x-2 disabled:opacity-50"
          >
            <ArrowLeft className="h-5 w-5" />
            <span>Change Number</span>
          </button>
        ...
        {/* Security Notice */}
        <div className="mt-6 p-4 bg-slate-800/40 border border-cyan-400/20 rounded-xl">
          <div className="flex items-start space-x-3">
            <Shield className="h-5 w-5 text-cyan-400 mt-0.5" />
            <div className="text-sm text-gray-300">
              <p className="font-semibold mb-1 text-white">Secure & Private</p>
              <p>
                Your phone number is verified securely and never shared with
                third parties.
              </p>
            </div>
          </div>
